...
async fn decrypt((encrypted, uid, key_id): &(Vec<u8>, String, String)) -> Result<(), tonic::Status> {
  let mut client = lib::client().await.map_err(|e| tonic::Status::from_error(e.into()))?;
  client
...
pub fn decryption_benchmark(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new(BENCHMARK_NAME, "decryption"), &(text, uid, key_id), |b, value | {
        b.to_async(&rt)
            .iter(|| decrypt(value));
    });
}
